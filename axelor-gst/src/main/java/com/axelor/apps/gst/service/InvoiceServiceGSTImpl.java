...
public class InvoiceServiceGSTImpl implements InvoiceServiceGST {
  ...
  //			isSameState = addressService.checkAddressStateForInvoice(companyAddress, invoiceAddress,
  // shippingAddress,
  ...
  //				invoiceLine = invoiceLineServiceGST.calculateInvoiceLine(invoiceLine, isSameState,
  // isNullAddress);
  ...
  public Invoice calculate(Invoice invoice) throws AxelorException {
    ...
    //			BigDecimal priceDiscounted = invoiceLineService.computeDiscount(invoiceLine,
    // invoice.getInAti());
    //
    ...
    //		invoice.setInvoiceLineList(invoiceLineList);
    //		// Create tax lines.
    //		List<InvoiceLineTax> invoiceTaxLines = (new TaxInvoiceLine(invoice,
    // invoice.getInvoiceLineList())).creates();
    //
  ...
  public Invoice setInvoiceDetails(Invoice invoice, List<Long> productIds, Integer partnerId)
    ...
    //		List<Product> productList = productRepository.all().filter("self.id in (?1)",
    // productIds).fetch();
    //		List<InvoiceLine> invoiceLineList =
    // invoiceLineServiceGST.getInvoiceLineFromProduct(productList);
    //		Partner partner = Beans.get(PartnerRepository.class).all().filter("self.id = ?",
    // partnerId).fetchOne();
    //
    //		invoice.setInvoiceLineList(invoiceLineList);
