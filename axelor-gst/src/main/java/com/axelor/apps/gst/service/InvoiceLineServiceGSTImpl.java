...
public class InvoiceLineServiceGSTImpl extends InvoiceLineProjectServiceImpl implements InvoiceLineServiceGST {
	...
	public Map<String, Object> fillProductInformation(Invoice invoice, InvoiceLine invoiceLine) throws AxelorException {
		...
		if (productInformation.get("taxLine") == null) {
			long id = (long) (taxRepository.findByCode("GST") != null ? taxRepository.findByCode("GST").getId() : 0);

			TaxLine taxLine = taxLineRepository.all().filter("self.value = :value and self.tax = :taxId")
					.bind("value", invoiceLine.getGstRate().divide(new BigDecimal(100))).bind("taxId", id).fetchOne();
			...
			invoiceLine.setGstRate(invoiceLine.getTaxLine().getValue().multiply(new BigDecimal(100)).setScale(2,
		...
		log.debug("company::" + invoice.getCompany() + "  Company address::" + invoice.getCompany().getAddress()
				+ " company state:: " + invoice.getCompany().getAddress().getState() + " invoice address:"
				+ invoice.getAddress() + " invoice address state::" + invoice.getAddress().getState());

		if (invoice.getCompany() == null || invoice.getCompany().getAddress() == null
				|| invoice.getCompany().getAddress().getState() == null || invoice.getAddress() == null
			...
			isSameState = addressService.checkAddressStateForInvoice(companyAddress, invoiceAddress, shippingAddress,
	...
	@Override
	public InvoiceLine calculateInvoiceLine(InvoiceLine invoiceLine, Boolean isInvoiceIsShipping,
		...
		if (!isNullAddress) {
			if (invoiceLine.getQty() != null && invoiceLine.getPrice() != null
		...
		finalGST = finalGST.add(invoiceLine.getIgst()).add(invoiceLine.getSgst()).add(invoiceLine.getCgst());
