...
public class SaleOrderLineServiceGSTImpl extends SaleOrderLineServiceSupplyChainImpl {
  ...
  public SaleOrderLine calculatesaleOrderLine(
    ...
    //          saleOrderLine.getPrice().multiply(saleOrderLine.getGstRate().divide(new
    // BigDecimal(100)));
  ...
  public TaxLine getTaxLine(SaleOrder saleOrder, SaleOrderLine saleOrderLine)
    ...
    //		try {
    //			taxLine = Beans.get(AccountManagementService.class).getTaxLine(saleOrder.getCreationDate(),
    //					saleOrderLine.getProduct(), saleOrder.getCompany(),
    //					saleOrder.getClientPartner().getFiscalPosition(), false);
    //		} catch (Exception e) {
    //
    ...
    //				long id = (long) (taxRepository.findByCode("GST") != null ?
    // taxRepository.findByCode("GST").getId()
    //						: 0);
    ...
    //				taxLine = taxLineRepository.all().filter("self.value = :value and self.tax = :taxId")
    //						.bind("value", saleOrderLine.getProduct().getGstRate().divide(new BigDecimal(100)))
    //						.bind("taxId", id).fetchOne();
